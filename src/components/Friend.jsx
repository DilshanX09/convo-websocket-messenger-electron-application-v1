...
const Friend = React.forwardRef(({ friend, selected_person, isActive, setSelectUser, handlePersonClick, typingStatus, unreadCount }, ref) => {
     ...
     return (
          <motion.div
               ...
               className={`flex items-center border-b ${theme == 'light' ? 'border-[#e4e4e4] first:border-t first:border-[#e6e6e6]' : 'border-[#2B2B2B] first:border-t first:border-[#2B2B2B]'} relative justify-between px-2 py-2 ${selected_person === friend.UUID
                    ? theme === 'dark'
                         ? 'bg-[#2B2B2B] border-none'
          ...
          >
               <div className="flex items-center">
                    ...
                    {
                         !friend.PROFILE_IMAGE_URL ? <img src={theme === 'light' ? user : user_white} className={`${theme === 'light' ? 'bg-gray-100' : 'bg-[#2b2b2b]'} ${friend.STATUS == 'Online' && 'border-green-500 border-[2px]'}  w-14 h-14 mr-3 rounded-full `} /> : <img
                              className={`w-14 h-14 mr-3 rounded-full ${friend.STATUS == 'Online' && 'border-green-500 border-[2px]'} `}
...

               {
                    friend.DATE && (
                         <span className="text-xs absolute right-2 top-3 pb-4 font-medium ">
                              <span className={`px-1 py-1 rounded-md ${theme === 'light' ? (!isActive && unreadCount > 0 ? 'text-green-500' : 'text-gray-700') : (!isActive && unreadCount > 0 ? 'text-green-300' : 'text-gray-400')}`}>
